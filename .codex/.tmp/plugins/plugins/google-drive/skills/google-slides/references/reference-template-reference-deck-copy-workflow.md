...
2. Read the provided deck with `get_presentation` and, when helpful, `get_presentation_outline`; record presentation id, title, slide count, page size, slide object IDs, layout IDs, masters, and revision id when present.
3. Fetch and inspect thumbnails for representative template/reference slides before choosing exemplars. This is required: do not choose final exemplars from outline text alone.
...
6. Build a slide plan mapping each target slide to an existing exemplar slide in the copied deck whenever possible. Use a copied-deck layout only when the layout itself has been inspected and is sufficient for the intended content.
7. For each target slide, build a slot map before writing. Record the existing object IDs, placeholder metadata, object type, geometry, current text/image/chart/table role, and whether each object is a reusable content slot, decorative element, footer/header, or obsolete source content.
...
11. Edit the duplicated or newly inserted slides with focused `batchUpdate` calls: clear and rewrite existing text boxes and table cells, replace placeholder or sample text scoped by `pageObjectIds`, replace images or charts in the intended existing footprint, and delete obsolete source/reference content.
...
15. Finish with full structural readback to verify the copied deck, not the source deck, is the final editable Google Slides deliverable. Use `get_presentation` or `get_slide` for every delivered slide; `get_presentation_outline` alone is not sufficient for the final template-use validator because it does not expose placeholder metadata or object structure.
...
- The new source material is truth for claims, metrics, examples, dates, and narrative.
...
- For text, prefer `deleteText` plus `insertText` on the existing text box, placeholder, or table cell. Preserve the object's transform and inherited style unless the source content cannot fit after reasonable shortening.
...
Report only the copied Google Slides deck as the deliverable. Name the source/template deck and copied deck identities verified by connector readback, summarize the slide creation strategy used, and call out any remaining slide that needed human design judgment.

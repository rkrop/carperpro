...
function lineInputIsValid(line: { productId: string; qty: number }): boolean {
  return (
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (!parsed.success) {
      res
        .status(400)
        .json({ error: "Pedido inválido", details: parsed.error.issues });
      return;
    }
    ...
    if (input.lines.length > MAX_ORDER_LINES) {
      res.status(400).json({
        error: `El pedido excede el máximo de ${MAX_ORDER_LINES} líneas`,
      });
      return;
    }
    if (!input.lines.every(lineInputIsValid)) {
      res
        .status(400)
        .json({ error: "El pedido contiene productos o cantidades inválidas" });
      return;
    }
    ...
    if (input.entrega === "envio" && !shippingAddress) {
      res
        .status(400)
        .json({ error: "La dirección de envío está incompleta o es inválida" });
      return;
    }
    ...
    if (userId) {
      try {
        ...
        logger.warn(
          { userId, err },
          "No se pudo aprovisionar la cuenta al crear el pedido",
        );
      }
    }
      ...
      order = await db.transaction(async (tx) => {
        ...
        const dbProducts = await tx
          .select({
            ...
            stock: productsTable.erpStockQty,
          })
          .from(productsTable)
        ...
        if (unknownIds.length > 0) {
          throw new OrderError(400, {
            error: "Productos no encontrados en catálogo",
            ids: unknownIds,
          });
        }
        ...
        if (notSellable.length > 0) {
          throw new OrderError(400, {
            error: "Algunos productos no están disponibles para venta",
        ...
        for (const l of input.lines) {
          requestedByProduct.set(
            l.productId,
            (requestedByProduct.get(l.productId) ?? 0) + normalizedQty(l.qty),
          );
        }
        const shortfalls = dbProducts.filter(
          (p) =>
            p.stock != null && (requestedByProduct.get(p.id) ?? 0) > p.stock,
        );
        if (shortfalls.length > 0) {
          const detail = shortfalls
            .map((p) => `${p.name} (disponible: ${p.stock})`)
            .join(", ");
    ...
    try {
      ...
      logger.warn(
        ...
        "Admintotal: push inmediato falló, pedido en cola para reintento",
      );
    }

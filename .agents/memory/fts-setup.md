...
- **search_vector is backfilled, not lazy anymore.** Two idempotent, batched (2000/tx) backfills touch rows where `search_vector IS NULL` so the trigger recomputes: `artifacts/api-server/src/lib/search-backfill.ts` runs on **every server boot** (so a publish indexes prod, which the agent can only read), and `seed-excel.mjs` (the canonical manual seed) does the same. Any insert/update (sync, webhooks, raw UPDATE) fires the BEFORE trigger, so the index self-heals.

# search_vector must live in the Drizzle schema

- `search_vector` (tsvector) is declared in `lib/db/src/schema/products.ts` via a `customType` (Drizzle has no native tsvector). It is omitted from `insertProductSchema` — the app never writes it; the `products_search_trigger` / `products_search_vector_update()` trigger+function populate it.
- **The trigger+function are now versioned in code** (`artifacts/api-server/src/lib/ensure-search-trigger.ts`, mirrored in `seed-excel.mjs`). `ensureSearchTrigger()` runs on every boot (before backfill) and `CREATE OR REPLACE`s the function so dev AND prod self-apply on publish. Keep the function body identical in both files.
- **Weights now include vehicles + specs at weight D** (lowest), below descripcion (C): the function appends `array_to_string(vehicles,' ')` and the `value`/`label` of each `specs` jsonb element so a search like "tsuru" finds products whose only match is the compatible-vehicle/spec field. ts_rank keeps these below name/SKU (A) and brand/oem (B).
- **Reindex trick:** `UPDATE products SET search_vector = NULL` does NOT leave NULLs — the BEFORE UPDATE trigger recomputes the vector in the same statement, so one UPDATE reindexes the whole catalog. `ensureSearchTrigger` does this only when it detects the function body changed (normalized prosrc compare); backfill then finds zero NULLs.

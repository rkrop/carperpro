...
// ── Direct OpenAI client (real API key) ─────────────────────────────────────
// Keyed by OPENAI_API_KEY and hitting api.openai.com directly. Used by the
// enrichment pipeline, which needs reliable credited throughput. Lazy + cached
// like getOpenAI; guard optional callers with isOpenAIDirectConfigured().
...
/**
 * Returns an OpenAI client built from OPENAI_API_KEY, talking to api.openai.com
 * directly. Throws if no
...
export function getOpenAIDirect(): OpenAI {
  ...
  if (!apiKey) {
    throw new Error(
      "OPENAI_API_KEY must be set to use the direct OpenAI client.",
    );
  }
